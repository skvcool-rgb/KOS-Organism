"""
Auto-generated learned engine: learned_b94a9452_1775414887
Task: b94a9452
Description: Evolved for b94a9452: SEQ(OVERLAY(SWAP(COLOR_MAX,COLOR_UNIQUE), DELETE_COLS_ZERO) -> SEQ(OVERLAY(CROP_TO_NONBG, RECOLOR(COLOR_MAX,COLOR_SECOND)) -> IF_COLOR(ORIG_COLOR_FG_2, DELETE_ROWS_ZERO, DELETE_COLS_ZERO)))
AST: ('SEQ', ('OVERLAY', ('SWAP', 'COLOR_MAX', 'COLOR_UNIQUE'), 'DELETE_COLS_ZERO'), ('SEQ', ('OVERLAY', 'CROP_TO_NONBG', ('RECOLOR', 'COLOR_MAX', 'COLOR_SECOND')), ('IF_COLOR', 'ORIG_COLOR_FG_2', 'DELETE_ROWS_ZERO', 'DELETE_COLS_ZERO')))
...
def apply_rule(grid, rule=None):
    ...
    state = grid.copy()
    state_a = state.copy()
    _c1, _c2 = _resolve_color("COLOR_MAX", _orig_grid), _resolve_color("COLOR_UNIQUE", _orig_grid)
    ...
    result_a = state.copy()
    state = state_a.copy()
    state[state == _resolve_color("COLOR_MAX", _orig_grid)] = _resolve_color("COLOR_SECOND", _orig_grid)
    result_b = state.copy()
    state = result_a.copy()
    mask = result_b != 0
    state[mask] = result_b[mask]
    if _resolve_color("COLOR_FG_2", _orig_grid) in state:
